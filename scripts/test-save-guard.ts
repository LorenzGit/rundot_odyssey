...
const realSave = JSON.stringify({
    ...
    settings: {},
    progress: { score: 0, coins: 777, level: 12, totalPlays: 40, ownedArrows: ["reed"] },
});
...
store.patch({ coins: store.get().coins + 5 });
...
await tick(2_300);
expect(store.get().coins === 777, `retry should apply the real save (coins ${store.get().coins})`);
expect(store.get().level === 12, "retry should restore level");
store.patch({ coins: 800 });
expect((await saveSystem.flush()) === true, "flush should succeed once the cloud read succeeded");
expect(JSON.parse(cloud.get(SAVE_KEY) ?? "{}").progress?.coins === 800, "verified flush should reach the cloud");

// 3. A save from a newer build is never overwritten.
const newer = JSON.stringify({ version: SAVE_VERSION + 1, settings: {}, progress: { coins: 5 }, futureField: true });
